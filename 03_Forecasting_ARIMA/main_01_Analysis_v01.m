...
%         fprintf('tsName: %s nY: %d; \n', tsName , nY);
% 
%         if i == 1
%             h1 = figure('Name','TS [last year] raw data 1'); % h1 = figure;
%             kFig = 1; grid on;
%         elseif i == 5         
%             %h2 = figure('Name','TS [last year] raw data 2'); % h2 = figure;
%             h1 = figure('Name','TS [last year] raw data 2'); % h1 = figure;
%             kFig = 2; grid on;
%         end    
% 
%         
...
%         if (nY ==1 )
%             title(strcat( tsName,' 2016-12'));
%         elseif (nY ==2 )
%             title(strcat( tsName,' 2016'));
%         elseif (nY ==3 )
%             title(strcat( tsName,' 2012-2016'));
%         end
%         
...
%             folder = 'Figures';
%             if (nY ==1 )
%                 filename = strcat( "timeSeriesPollutionMadrid_2016_12","_",num2str(kFig));
%             elseif (nY ==2 )
%                 filename = strcat( "timeSeriesPollutionMadrid_2016","_",num2str(kFig));
%             elseif (nY ==3 )
%                 filename = strcat( "timeSeriesPollutionMadrid_2012-2016","_",num2str(kFig));
%             end
%                 
...
%     % suptitle('Time Series')  % not valid for R2017b
% 
%     % h1 = figure('Name','TS last year raw data log scale'); %h2 = figure;
%     % for i = 1:length(tsLastYearDataCellArray)
%     %     tsData = tsLastYearDataCellArray{1,i};
%     %     tsName = tsNamesCellArray{1,i};
%     %     subplot(2,2,i), plot(tsData);  
%     %     h2gca = gca; h2gca.YScale = 'log';
%     %     title(string(tsName) + ' log scale ' );
%     % end
%     % clear h1 tsData tsName variance;
%     % % suptitle('log y scale')
% 
...
%% TS DATA LAST MONTH aprox
% % Revise this source code snip for 8 time series (2017 11 13th)
% titleSubString =  ' Last month + hist';
% for i=1:4 
%    
%     tsData = tsLastMonthDataCellArray{1,i};
%     tsName = string(tsNamesCellArray{1,i});
%     
%     variance = var(tsData);
%     
%     h1 = figure('Name','TS last month raw data + hist'); % h4 = figure;
%     subplot(2,2, [1 2]), plot(tsData),  title(strcat( string(tsName), titleSubString  ) ); 
%     subplot(2,2,3), histogram(tsData,10); 
%     subplot(2,2,4), histogram(tsData,100)
%     
% end
% clear h1 tsData tsName variance;
% 
% h1 = figure('Name','TS last month raw data'); % h4 = figure;
% titleSubString =  ' raw data Last month';
% for i=1:4 
%     
%     tsData = tsLastMonthDataCellArray{1,i};
%     tsName = string(tsNamesCellArray{1,i});
...
%     variance = var(tsData);
%         
%     subplot(2,2,i), plot(tsData),  title( strcat( string(tsName), titleSubString  ) ); 
%     
% end
% clear h1 tsData tsName variance;
...
%% TS ANALYSIS 01 One TS (N02- FdezLadreda) Last Year + Last Month => ACF + PACF=====================================================================

% % LAST MONTH from 2015 --------------------------------------
% % Get Data 
% 
% % for indexSeries = 1:
% % end
% 
% tsData = tsLastMonthDataCellArray{2};
% tsName = tsNamesCellArray{2};
% 
% % Autocorrelation
% showPlot1 = 1;
% showPlot2 = 2;
% 
% sInfo = 'data:lastMonth';
% %[ACF,lags,bounds] = getAutocorrelation(   tsData , tsName , numLags , showPlot); 
% fprintf('\n %s -> Last Month ACF (lasgs %d) \n',tsName,numLags);
% [ ACF_01  , acf_lags_01_a , acf_bounds_01 , h1_scatter , h1_acf ] = studyAutocorrelation( tsData , tsName , sInfo, numLags , 0, showPlot2); 
% 
% fprintf('\n %s -> Last Month PACF (lasgs %d) \n',tsName,numLags);
% [ PACF_01_a , pacf_lags_01 , pacf_bounds_01, h1_scatter , h1_pacf ] = studyPartialAutocorrelation( tsData , tsName , sInfo, numLags , showPlot1, showPlot2); 
% 
% pause(2);
% h1_acf.Visible = 'off'; 
% h1_scatter.Visible = 'off';
% h1_pacf.Visible = 'off';
% 
% folder = 'Figures'; format = '-dpdf';
% filename = strcat(tsName,'_','DataLastMonth','_','laggedScatterPlot');
% error = saveFigure( h1_scatter , format , folder, filename);
% 
% filename = strcat(tsName,'_','DataLastMonth','_','acfPlot');
% error = saveFigure( h1_acf , format , folder, filename);
% 
% filename = strcat(tsName,'_','DataLastMonth','_','pacfPlot');
% error = saveFigure( h1_pacf , format , folder, filename);
% 
% 
% 
% % LAST YEAR ( i.e. 2015 ) --------------------------------------
% tsData = tsLastYearDataCellArray{2};
% tsName = tsNamesCellArray{2};
% % Autocorrelation
% 
% showPlot2 = 1;
...
% numWeeks = 2; 
% numLags2 = hoursPerDay * daysPerWeek * numWeeks; % numLags2 = 200;
% 
% sInfo = 'data:lastYear';
% 
% %[ACF,lags,bounds] = getAutocorrelation(   tsData , tsName, numLags2  , showPlot); 
% fprintf('\n %s -> Last Year ACF (lasgs %d) \n',tsName,numLags);
% [ ACF_02 , acf_lags_02 , acf_bounds_02, h2_scatter , h2_acf ] = studyAutocorrelation( tsData , tsName, sInfo, numLags2 , 0 , showPlot2); 
% 
% fprintf('\n %s -> Last Year PACF (lasgs %d) \n',tsName,numLags);
% [ PACF_02 , pacf_lags_02 , pacf_bounds_02, h2_scatter , h2_pacf ] = studyPartialAutocorrelation( tsData , tsName , sInfo, numLags2 ,showPlot1, showPlot2); 
% 
% 
% pause(2);
% h2_acf.Visible = 'off'; 
% h2_scatter.Visible = 'off';
% h2_pacf.Visible = 'off';
% 
% filename = strcat(tsName,'_','DataLastYear','_','laggedScatterPlot');
% error = saveFigure( h2_scatter , format , folder, filename);
% 
% filename = strcat(tsName,'_','DataLastYear','_','acfPlot');
% error = saveFigure( h2_acf , format , folder, filename);
% 
% filename = strcat(tsName,'_','DataLastYear','_','pacfPlot');
% error = saveFigure( h2_pacf , format , folder, filename);
% 
% 
% 
% % ARIMA Model
% % model  = arima('Constant',0,'D',1,'Seasonality',7,...
% %               'MALags',1,'SMALags',7);
