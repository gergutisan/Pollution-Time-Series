function [result] = ann4tsForecast_v02( parameters )
  ...
  result.comment = '-'; 
  strNumExp = sprintf('%06i',parameters.iCombFactLevel);
  ...
  learningRate      = parameters.learningRate;      %learningRate   = 0.1;    % 0.2 0.1 0.05 0.01                          
  ...
  %% Show Target & Outputs -------------------------
  if (parameters.showTrainValTestTargetOutput)
      ...
      TrVaTeTargetOutputFigFilename = strcat(parameters.folder_Results,'/',...
                                             tsName,'-TrVaTe+TarOut-',strNumExp,'.fig');
  ...
  if (parameters.showValTargetOutput)
      ...
      legend('target','output'); 
      title(strcat('(Validation)','--',parameters.tsFilename));
      
       
      testTargetOutputFigFilename = strcat(parameters.folder_Results,'/',...
                                           tsName,'-Val+TarOut-',strNumExp,'.fig');
      savefig(testTargetOutputFigFilename);
      %testTargetOutputFigFilename = strcat(parameters.folder_Results,'/',...
  ...
  if (parameters.showTestTargetOutput)
