...
elseif ( horizon > 1 )
    ...
    for iVal = 1:numel(valTargets)
        ...
        output = sim(net01,inputPattern'); 
        target = knownValTargets(iVal); 
        
        valH2Targets(iVal) = target;
        valH2Outputs(iVal) = output;
        
        % new input pattern 
        v = inputPattern(2:end); 
        inputPattern = cat( 2 , v , output ); clear v;
    ...
    % note: first two instructions for debugging purposes
    ...
    smapeValForecast_03 = 100 * ( 2* sum(errorEachPeriod) ) / numel( valTargets ); 
    ...
    smapeTeForecast_03 = 100 * ( 2* sum(errorEachPeriod) ) / numel( testTargets ); 
    clear errorEachPeriod;
