function ensembleResults = func_01_GetEnsemble( tsNamesCellArray , parameters , doeResults , iIterDoE)
    %{
...
%% SET FOLDERs AND PATH     ===============================================================
    ...
    % parameters.bestByRMSE  = 1;  % 1/0
    % parameters.bestBySMAPE = 0;  % 0/1 
    ...
    ensembleResults.parameters = parameters;
    ...
    % GET THE BEST from DoE experiment iteration 
    ...
    ensembleResults.ensembleElementsAllTsOneIter = ensembleElementsAllTsOneIter; 
    ...
    for indexTsName = 1:nTimeSeries
        ...
        filename = fullfile(folderFigures,strcat('bDoE_FLC_',tsNamesCellArray{indexTsName},'.fig'));
        saveas ( h1 , filename ); 

        filename = fullfile(folderFigures,strcat('bDoE_FLC_',tsNamesCellArray{indexTsName},'.png'));
        saveas ( h1 , filename ); 
    end
        ...
        for iTsType = 1:nTsTypes
            ...
            switch iTsType 
                case 1 
                    ...
                    net = ensembleElementsAllTsOneIter{r,c}.net;
    ...
    ensembleResults.Elements_valTarget4RawData    = valTarget4RawData;
    ensembleResults.Elements_testTarget4RawData   = testTarget4RawData;
    ensembleResults.Elements_valForecast4RawData  = valForecast4RawData;
    ensembleResults.Elements_testForecast4RawData = testForecast4RawData;
    ...
    %%  LETS COMPUTE THE ERRROR ON ORIGINAL OBSERVATIONS OF EACH ELEMENT OF THE ENSEMBLE  ========
    for indexTsName = 1:nTimeSeries  
        for iTsType = 1:nTsTypes
    ...
    ensembleResults.Elements_rmseVal   = rmseVal;
    ensembleResults.Elements_rmseTest  = rmseTest;
    ensembleResults.Elements_smapeVal  = smapeVal;
    ensembleResults.Elements_smapeTest = smapeTest;
    ...
    ensembleResults.weightsSmape = weightsSmape;
    ...
    for indexTsName = 1:nTimeSeries  
        ...
        nValTime = numel( valForecast4RawData{ indexTsName , 1} );
        ensembleValForecast{indexTsName} = zeros( nValTime, 1 );
                
        nTestTime = numel( testForecast4RawData{indexTsName , 1} );
        ensembleTestForecast{indexTsName} = zeros( nTestTime, 1 );
        

        for iTsType = 1:nTsTypes
            
            ensembleValForecast{indexTsName}  = ensembleValForecast{indexTsName} + ...
                                                weightsRmse( indexTsName , iTsType ) * valForecast4RawData{ indexTsName , iTsType } ;    

            ensembleTestForecast{indexTsName} = ensembleTestForecast{indexTsName} + ...
                                                weightsRmse( indexTsName , iTsType ) * testForecast4RawData{ indexTsName , iTsType } ;

        end
        
        % Compute the error ON VAL from output of the ensemble
        % VAL       ============================================================
        targets  = valTarget4RawData{ indexTsName};
        forecast = ensembleValForecast{ indexTsName};
                
        rmseTmp  = ( 1/numel( targets ) ) * sqrt( sum( ( targets -  forecast ).^2 ) );
        errorEachPeriod = ( abs( forecast - targets ) ) ./ ( abs(forecast) + abs(targets) );
        smapeTmp = 100 * ( 2* sum(errorEachPeriod ) ) / numel( targets );  

        rmseValEnsemble(  indexTsName ) = rmseTmp;
        smapeValEnsemble( indexTsName ) = smapeTmp;
        ...
        % TEST       ============================================================
        targets  = testTarget4RawData{ indexTsName};
        forecast = ensembleTestForecast{   indexTsName};
    ...
    fprintf('Ensemble rmse  Val\n');  disp(rmseValEnsemble);
    fprintf('Ensemble smape Val\n');  disp(smapeValEnsemble);
    ...
    h4 = figure( 'Name' , 'EnsembleWeightsRMSE Forecast on test subset with ' );
    for indexTsName = 1:nTimeSeries  
        ...
        title(strcat('EnsembleWeightsRMSE -',tsNamesCellArray{indexTsName}));

    end
    
     filename = fullfile(folderFigures,strcat('EnsembleWR_Forecast','.fig'));
     saveas ( h4 , filename ); 
     filename = fullfile(folderFigures,strcat('EnsembleWR_Forecast','.png'));
     saveas ( h4 , filename ); 
    ...
    for indexTsName = 1:nTimeSeries  
        ...
        for iTsType = 1:nTsTypes
            ...
            ensembleValForecast{indexTsName} = ensembleValForecast{indexTsName} + ...
                                               weightsSmape( indexTsName , iTsType ) * valForecast4RawData{ indexTsName , iTsType };
    ...
    for indexTsName = 1:nTimeSeries  
        ...
        title(strcat('EnsembleWeightsSMAPE -',tsNamesCellArray{indexTsName}));
        
    end
    filename = fullfile(folderFigures,strcat('EnsembleWS_Forecast','.fig'));
    saveas ( h4 , filename ); 
    filename = fullfile(folderFigures,strcat('EnsembleWS_Forecast','.png'));
    saveas ( h4 , filename ); 
    % end
    % --- End of iterExp -------
    %% TO STOP the script 
